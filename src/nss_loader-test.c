...
main (void)
{
  ...
  nss_handle = dlopen (".libs/libnss_nisplus.so.2", RTLD_NOW);
  if (!nss_handle)
    {
      fprintf (stderr, "dlopen(\".libs/libnss_nisplus.so.2\") failed:\n%s\n",
	       dlerror ());
      return 1;
    }
  ...
  if ((error = dlerror ()) != NULL)
    {
      fprintf (stderr, "dlsym(\"_nss_nisplus_getpwnam_r\") failed: %s\n",
	       error);
      dlclose (nss_handle);
      return 1;
    }
